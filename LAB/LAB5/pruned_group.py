...
def train(args):
    ...
    # Charger le `state_dict` nettoyé dans le modèle
    model.load_state_dict(filtered_state_dict, strict=False)
    # Prune the model
    apply_l1_structured_pruning(model, amount=args.prate)
    model = model.to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=3, verbose=True)
    # Initialize Weights & Biases
    wandb.init(project="deep-learning-lab3", config=args.__dict__ , name="pruned_group_8", job_type="training_test")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='/opt/img/effdl-cifar10/', help='Path to data')
    parser.add_argument('--batch_size', type=int, default=64, help='Batch size')
    parser.add_argument('--learning_rate', type=float, default=0.001, help='Learning rate')
    parser.add_argument('--weight_decay', type=float, default=0.0005, help='Weight decay')
    parser.add_argument('--epochs', type=int, default=30, help='Number of epochs')
    parser.add_argument('--load_path', type=str, default='./group_8.pth', help='Path to load model')
    parser.add_argument('--save_path', type=str, default='./group_8_pruned.pth', help='Path to save model')
    parser.add_argument('--prate', type=float, default=0.2, help='Pruning rate')
